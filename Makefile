...
# Output from Step 5: hidden marker file indicating sensitivity analysis is complete
SENSITIVITY_OUTPUTS := $(MISC_DIR)/.sensitivity_done
...
# Target: create marker file indicating sensitivity analysis is complete
# Dependencies: runs if program 5 OR imputed data OR outlier results are newer than marker
$(SENSITIVITY_OUTPUTS): $(CODE_DIR)/5.\ AFC_sensitivity_analysis.R $(IMPUTED_DATA) $(CHEM_OUTPUTS)
	# Print status message to terminal
	@echo "======================================"
	@echo "Step 5: Sensitivity Analysis"
	@echo "This may take a long time (parallel processing)..."
	@echo "======================================"
	# Change to code directory and run program 5 using Rscript
	cd $(CODE_DIR) && $(R) "5. AFC_sensitivity_analysis.R"
	# Create misc directory if it doesn't exist
	@mkdir -p $(MISC_DIR)
	# Create hidden marker file to indicate this step is complete
	@touch $(SENSITIVITY_OUTPUTS)
...
$(REPORT): $(CODE_DIR)/EARTH_Analysis_Report.qmd $(ANALYSIS_OUTPUTS) $(SENSITIVITY_OUTPUTS)
	# Print status message to terminal
	@echo "======================================"
...
.PHONY: step0 step1 step2 step3 step4 step5 report
...
# Shortcut to run only Step 5 (sensitivity analysis) - also runs prior steps if needed
step5: $(SENSITIVITY_OUTPUTS)

# Shortcut to generate only the report - assumes all steps 0-5 are complete
report: $(REPORT)
...
help:
	...
	@echo "  make step5        - Sensitivity analysis (very slow, parallel processing)"
	@echo ""
